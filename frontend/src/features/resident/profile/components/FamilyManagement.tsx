...
const FamilyManagement: React.FC<FamilyManagementProps> = ({ familyMembers, onUpdate, colorScheme }) => {
  const addMember = () => {
    onUpdate([...familyMembers, { name: '', relation: '' }]);
  };
  ...
  return (
    <View className="mb-4 bg-gray-50/50 dark:bg-zinc-900/40 p-5 rounded-lg border border-gray-100 dark:border-zinc-800">
      <View className="flex-row items-center justify-between mb-4">
        <View className="flex-row items-center">
          ...
          <Text className="text-[14px] font-satoshi-bold text-gray-900 dark:text-zinc-50 tracking-tight">Family Members</Text>
        </View>
        <TouchableOpacity
          onPress={addMember}
      ...
      {familyMembers.length === 0 ? (
        <View className="py-4 items-center bg-white/50 dark:bg-zinc-900/50 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800">
          <Text className="text-gray-400 dark:text-zinc-500 text-[12px] font-satoshi-medium">No family members added</Text>
        </View>
      ) : (
        familyMembers.map((m, i) => (
          <View key={i} className="flex-row items-center gap-x-2 mb-3 bg-white dark:bg-zinc-900 p-2 rounded-2xl border border-gray-50 dark:border-zinc-800 shadow-sm">
            <View className="flex-1">
              ...
              <TextInput
                value={m.relation}
                onChangeText={(t) => updateMember(i, 'relation', t)}
